...
export const Web3Provider: React.FC<Web3ProviderProps> = ({ children }) => {
  ...
  const switchNetwork = async () => {
    ...
    console.log('Attempting to switch to network:', { chainIdHex, chainId: NETWORK_CONFIG.chainId });
    ...
    try {
      ...
      toast.success(`Successfully switched to ${NETWORK_CONFIG.chainName}`);
      
      // Update network status after switching
      const web3Provider = new ethers.providers.Web3Provider(window.ethereum);
      const { chainId: updatedChainId } = await web3Provider.getNetwork();
      setChainId(updatedChainId);
      setIsCorrectNetwork(checkCorrectNetwork(updatedChainId));
      ...
      if (switchError.code === 4902) {
        try {
          ...
          toast.success(`Added ${NETWORK_CONFIG.chainName} to your wallet`);
